...
type Tab = 'home' | 'invest' | 'wallet' | 'profile'
const plans = [
...
function App() {
  ...
  const [modal, setModal] = useState<'register' | 'login' | 'invest' | null>(null)
  ...
  return <div className="site-shell">
    <header className="topbar"><button className="brand" onClick={() => setTab('home')} aria-label="bKash Investor home"><span className="brand-mark">b</span><span><b>bKash</b> Investor</span></button><nav className="desktop-nav" aria-label="Main navigation"><button className={tab==='home'?'active':''} onClick={()=>setTab('home')}>{language==='bn'?'হোম':'Home'}</button><button className={tab==='invest'?'active':''} onClick={()=>setTab('invest')}>{language==='bn'?'বিনিয়োগ':'Invest'}</button><button className={tab==='wallet'?'active':''} onClick={()=>setTab('wallet')}>{language==='bn'?'পোর্টফোলিও':'Portfolio'}</button><button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>● {language==='bn'?'প্রোফাইল':'Profile'}</button></nav><div className="header-actions"><div className="language-switch" role="group" aria-label="Language"><button className={language==='bn'?'active':''} onClick={() => setLanguage('bn')}>বাংলা</button><button className={language==='en'?'active':''} onClick={() => setLanguage('en')}>English</button></div>{loggedIn?<button className="login-button" onClick={()=>{sessionStorage.removeItem('bkash-investor-session');setLoggedIn(false);setTab('profile');notify(language==='bn'?'লগআউট হয়েছে':'Logged out')}}>{language==='bn'?'লগআউট':'Logout'}</button>:<button className="login-button" onClick={()=>setModal('login')}>{language==='bn'?'লগইন':'Login'}</button>}</div></header>
    <main>
      {tab === 'home' && <>
        <section className="hero-section"><div className="hero-copy"><span className="eyebrow">বিশুদ্ধ স্বর্ণে, নিরাপদ ভবিষ্যৎ</span><h1>মাত্র ৳৫,০০০ থেকে<br/><em>স্বর্ণে বিনিয়োগ</em></h1><p>আপনার টাকায় যাচাইকৃত ২২ ক্যারেট স্বর্ণ কিনুন। স্বর্ণের বাজারদরের সাথে সম্পদের মূল্য বাড়ান এবং দেশের gold-business SME-গুলোর অংশীদার হোন।</p><div className="hero-actions"><button className="primary" onClick={() => setTab('invest')}>গোল্ডে বিনিয়োগ করুন</button><button className="ghost" onClick={() => document.getElementById('how')?.scrollIntoView({ behavior: 'smooth' })}>কীভাবে কাজ করে</button></div><div className="trust-line"><span>✓ যাচাইকৃত ২২ ক্যারেট স্বর্ণ</span><span>✓ স্বচ্ছ মূল্য ও নিরাপদ ভল্ট</span></div></div><div className="hero-visual gold-visual"><div className="gold-coin">GOLD<small>22K</small></div><div className="balance-card"><small>আজকের ২২ ক্যারেট মূল্য</small><strong>৳ ১,৭২,৫০০</strong><span>● আজকের লাইভ বাজারদর</span></div><div className="float-card fc-one"><b>৳৫,০০০</b><span>সর্বনিম্ন বিনিয়োগ</span></div><div className="float-card fc-two">🔒 সুরক্ষিত ভল্ট</div></div></section>
        <section className="metrics"><div><b>১২.৮ কেজি+</b><span>স্বর্ণ সংরক্ষিত</span></div><div><b>২,৪৫০+</b><span>গোল্ড বিনিয়োগকারী</span></div><div><b>৳৫,০০০</b><span>সর্বনিম্ন বিনিয়োগ</span></div><div><b>২৪/৭</b><span>পোর্টফোলিও ট্র্যাকিং</span></div></section>
        ...
        <TrustCompliance language={language}/>
        <section className="section plans-section"><div className="section-heading"><div><span>আপনার স্বর্ণ, আপনার সম্পদ</span><h2>গোল্ড ইনভেস্টমেন্ট প্ল্যান</h2></div><button onClick={() => setTab('invest')}>সব দেখুন →</button></div><div className="plan-grid">{plans.map(plan => <PlanCard key={plan.name} plan={plan} onClick={() => { setSelectedPlan(plan); setModal('invest') }}/>)}</div></section>
        <section className="section opportunity-section"><div className="section-heading"><div><span>আপনার অভিজ্ঞ ব্যবসার নেটওয়ার্ক</span><h2>গোল্ড-ব্যাকড SME সুযোগ</h2></div><div className="verified-head">✓ ব্যবসা যাচাই সম্পন্ন</div></div><div className="opportunity-grid">{opportunities.map(item => <article className="opportunity" key={item.title}><div className="op-img" style={{background:item.tint}}><span>{item.tag}</span><b className="gold-symbol">GOLD BAR</b></div><div className="op-body"><small>✓ ব্যবসা ও স্বর্ণ যাচাইকৃত</small><h3>{item.title}</h3><p>{item.desc}</p><div className="progress"><i style={{width:`${item.raised}%`}}/></div><div className="fund-row"><span>{item.raised}% সংগ্রহ</span><b>{item.amount}</b></div><button className="op-button" onClick={()=>notify(`${item.title} বিস্তারিত—ডেমো মোড`)}>বিস্তারিত ও ঝুঁকি দেখুন →</button></div></article>)}</div></section>
        ...
        <PaymentSection compact onSelect={(name) => notify(`${name} পেমেন্ট মাধ্যম নির্বাচিত`)} />
        <section className="cta"><div><span>আজকের স্বর্ণ, আগামীর সম্পদ</span><h2>মাত্র ৳৫,০০০ দিয়ে Gold Investor হোন</h2><p>স্বচ্ছ মূল্য, যাচাইকৃত স্বর্ণ এবং অভিজ্ঞ gold-business network।</p></div><button className="primary white" onClick={() => setModal('register')}>ফ্রি অ্যাকাউন্ট খুলুন →</button></section>
      </>}
    ...
    <nav className="bottom-nav">{([['home','⌂','হোম'],['invest','↗','বিনিয়োগ'],['wallet','▣','পোর্টফোলিও'],['profile','●','প্রোফাইল']] as const).map(([id,icon,label])=><button key={id} className={tab===id?'active':''} onClick={()=>{setTab(id);window.scrollTo({top:0,behavior:'smooth'})}}><b>{icon}</b><span>{label}</span></button>)}</nav>
    {modal === 'register' && <RegisterForm language={language} onClose={() => setModal(null)} onSuccess={(name) => { sessionStorage.setItem('bkash-investor-session','active'); setLoggedIn(true); setModal(null); setTab('profile'); notify(language === 'bn' ? `${name}, আপনার অ্যাকাউন্ট তৈরি হয়েছে!` : `Welcome ${name}, your account is ready!`) }} />}
    {modal === 'login' && <LoginForm language={language} onClose={()=>setModal(null)} onSuccess={(name)=>{sessionStorage.setItem('bkash-investor-session','active');setLoggedIn(true);setModal(null);setTab('profile');notify(language==='bn'?`স্বাগতম, ${name}`:`Welcome back, ${name}`)}} onRegister={()=>setModal('register')} />}
    {modal === 'invest' && <div className="modal-backdrop" onMouseDown={(e)=>e.currentTarget===e.target&&setModal(null)}><div className="modal" role="dialog" aria-modal="true"><button className="close" onClick={()=>setModal(null)}>×</button><h2>{selectedPlan.name}</h2><p>{selectedPlan.risk} • সর্বনিম্ন {selectedPlan.minimum}</p><label>বিনিয়োগের পরিমাণ (৳)<input type="number" value={amount} min="5000" step="500" onChange={e=>setAmount(e.target.value)} /></label><div className="estimate"><span>আনুমানিক স্বর্ণের পরিমাণ</span><b>{((Number(amount)||0)/goldPrice*11.664).toFixed(3)} গ্রাম</b></div><button className="primary full" disabled={Number(amount)<5000} onClick={()=>{setModal(null);notify('ডেমো গোল্ড অর্ডার তৈরি হয়েছে')}}>গোল্ড অর্ডার নিশ্চিত করুন</button><small>ন্যূনতম ৳৫,০০০ • চূড়ান্ত ওজন কেনার সময়কার বাজারদরে নির্ধারিত হবে।</small></div></div>}
