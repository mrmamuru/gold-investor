...
function App() {
  ...
  const [modal, setModal] = useState<'register' | 'login' | 'invest' | 'orderPayment' | 'payment' | 'cardInfo' | null>(null)
  ...
  const submitManualPayment = async () => {
    ...
    if (!user) return setPaymentError(language==='bn'?'আবার লগইন করে চেষ্টা করুন':'Please log in again')
    const ext = paymentProof.name.split('.').pop()?.toLowerCase() || 'jpg'
    const proofPath = `${user.id}/${crypto.randomUUID()}.${ext}`
    const {error:uploadError} = await supabase.storage.from('payment-proofs').upload(proofPath,paymentProof,{contentType:paymentProof.type,upsert:false})
    if(uploadError) return setPaymentError(language==='bn'?`Screenshot upload হয়নি: ${uploadError.message}`:uploadError.message)
    const {error:requestError} = await supabase.from('payment_requests').insert({user_id:user.id,method:selectedPaymentMethod,amount:Number(paymentAmount),transaction_id:transactionId.trim(),proof_path:proofPath})
    if(requestError){await supabase.storage.from('payment-proofs').remove([proofPath]);return setPaymentError(language==='bn'?`Request জমা হয়নি: ${requestError.message}`:requestError.message)}
    setModal(null)
  ...
  useEffect(()=>{supabase.auth.getSession().then(({data})=>{if(data.session){sessionStorage.setItem('bkash-investor-session','active');setLoggedIn(true)}});const {data}=supabase.auth.onAuthStateChange((_e,s)=>{setLoggedIn(Boolean(s));if(!s)sessionStorage.removeItem('bkash-investor-session')});return()=>data.subscription.unsubscribe()},[])
  ...
  return <div className="site-shell">
    ...
    {modal === 'register' && <BackendRegisterForm language={language} onClose={() => {setModal(null);setPendingInvestment(false);setPendingPlan(null);setPendingPayment('')}} onSuccess={(name) => completeAuthentication(name)} onLogin={()=>setModal('login')} />}
    {modal === 'login' && <BackendLoginForm language={language} onClose={()=>{setModal(null);setPendingInvestment(false);setPendingPlan(null);setPendingPayment('')}} onSuccess={(name)=>completeAuthentication(name,true)} onRegister={()=>setModal('register')} />}
    ...
    {modal === 'cardInfo' && <div className="modal-backdrop" onMouseDown={e=>e.currentTarget===e.target&&setModal(null)}><div className="modal card-info-modal" role="dialog" aria-modal="true"><button className="close" onClick={()=>setModal(null)}>×</button><span className="checkout-label">CARD SECURITY</span><h2>{language==='bn'?'কার্ড পেমেন্ট':'Card payment'}</h2><p>{language==='bn'?'Visa, Mastercard, Amex ও Nexus দিয়ে সরাসরি পেমেন্ট চালু করতে অনুমোদিত merchant gateway প্রয়োজন।':'An authorised merchant gateway is required to accept Visa, Mastercard, Amex and Nexus payments.'}</p><div className="card-safety-note"><b>!</b><span>{language==='bn'?'নিরাপত্তার জন্য আমরা card number, CVV, PIN বা OTP manually গ্রহণ করি না।':'For security, we never collect card numbers, CVV, PIN or OTP manually.'}</span></div><button className="primary full" onClick={()=>openPayment('Bank Transfer')}>{language==='bn'?'ব্যাংক ট্রান্সফার ব্যবহার করুন':'Use bank transfer'} →</button><button className="text-button" onClick={()=>setModal(null)}>{language==='bn'?'এখন নয়':'Not now'}</button></div></div>}
...
function BackendLoginForm({language,onClose,onSuccess,onRegister}:{language:'bn'|'en';onClose:()=>void;onSuccess:(name:string)=>void;onRegister:()=>void}) {
  const bn=language==='bn',[email,setEmail]=useState(''),[password,setPassword]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false)
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);setError('');const {data,error}=await supabase.auth.signInWithPassword({email,password});setBusy(false);if(error)return setError(bn?'Email অথবা password সঠিক নয়':'Incorrect email or password');const {data:profile}=await supabase.from('profiles').select('full_name,phone').eq('id',data.user.id).single();const name=profile?.full_name||data.user.user_metadata?.full_name||'Customer';localStorage.setItem('bkash-investor-user',JSON.stringify({name,phone:profile?.phone||''}));onSuccess(name)}
  return <div className="modal-backdrop" onMouseDown={e=>e.currentTarget===e.target&&onClose()}><form className="modal login-form" onSubmit={submit}><button type="button" className="close" onClick={onClose}>×</button><h2>{bn?'লগইন করুন':'Log in'}</h2><p>{bn?'আপনার Gold Investor অ্যাকাউন্টে প্রবেশ করুন':'Access your Gold Investor account'}</p><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email" required placeholder="you@example.com"/></label><label>{bn?'Password':'Password'}<input value={password} onChange={e=>setPassword(e.target.value)} type="password" autoComplete="current-password" required minLength={6}/></label>{error&&<div className="form-error">⚠ {error}</div>}<button className="primary full" disabled={busy}>{busy?(bn?'লগইন হচ্ছে…':'Signing in…'):(bn?'লগইন':'Log in')}</button><button className="text-button" type="button" onClick={onRegister}>{bn?'অ্যাকাউন্ট নেই? তৈরি করুন':'No account? Create one'}</button></form></div>
}

function BackendRegisterForm({language,onClose,onSuccess,onLogin}:{language:'bn'|'en';onClose:()=>void;onSuccess:(name:string)=>void;onLogin:()=>void}) {
  const bn=language==='bn',[name,setName]=useState(''),[phone,setPhone]=useState(''),[email,setEmail]=useState(''),[password,setPassword]=useState(''),[confirm,setConfirm]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false)
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setError('');if(name.trim().length<2)return setError(bn?'সঠিক নাম লিখুন':'Enter your full name');if(!/^01[3-9]\d{8}$/.test(phone))return setError(bn?'সঠিক ১১ সংখ্যার মোবাইল নম্বর লিখুন':'Enter a valid mobile number');if(password.length<8)return setError(bn?'Password কমপক্ষে ৮ অক্ষরের দিন':'Use at least 8 characters');if(password!==confirm)return setError(bn?'দুটি password মিলছে না':'Passwords do not match');setBusy(true);const {data,error}=await supabase.auth.signUp({email,password,options:{data:{full_name:name.trim(),phone}}});setBusy(false);if(error)return setError(error.message);if(!data.session)return setError(bn?'আপনার email-এর confirmation link খুলে তারপর লগইন করুন':'Confirm your email, then log in');localStorage.setItem('bkash-investor-user',JSON.stringify({name:name.trim(),phone}));onSuccess(name.trim())}
  return <div className="modal-backdrop" onMouseDown={e=>e.currentTarget===e.target&&onClose()}><form className="modal register-form" onSubmit={submit}><button type="button" className="close" onClick={onClose}>×</button><h2>{bn?'অ্যাকাউন্ট খুলুন':'Create account'}</h2><p>{bn?'নিরাপদ payment ও portfolio-এর জন্য তথ্য দিন':'Create your secure payment and portfolio account'}</p><label>{bn?'পুরো নাম':'Full name'}<input value={name} onChange={e=>setName(e.target.value)} required autoComplete="name"/></label><label>{bn?'মোবাইল নম্বর':'Mobile number'}<input value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,'').slice(0,11))} required inputMode="numeric" placeholder="01XXXXXXXXX"/></label><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email" placeholder="you@example.com"/></label><label>{bn?'Password তৈরি করুন':'Create password'}<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={8} autoComplete="new-password"/></label><label>{bn?'Password আবার লিখুন':'Confirm password'}<input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} required minLength={8} autoComplete="new-password"/></label>{error&&<div className="form-error">⚠ {error}</div>}<button className="primary full" disabled={busy}>{busy?(bn?'তৈরি হচ্ছে…':'Creating…'):(bn?'অ্যাকাউন্ট তৈরি করুন':'Create account')}</button><button type="button" className="text-button" onClick={onLogin}>{bn?'আগে থেকেই অ্যাকাউন্ট আছে? লগইন':'Already registered? Log in'}</button></form></div>
}
